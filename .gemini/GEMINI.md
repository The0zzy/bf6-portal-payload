# AI Coding Agent Instructions for Battlefield 6 Portal Scripting

## Project Overview

This is a TypeScript template for creating Battlefield 6 Portal experiences (custom game modes/mods).
Code is compiled via a custom bundler and manually uploaded to Portal's web editor for testing.
...
- `levels/` - Spatial Editor JSON map files (optional)
  - `exports/` - Exported level files in json format to be uploaded to the Portal Web Editor (optional)
  - `scenes/` - Scene files in tscn format to be imported into the Godot Editor (optional)
- `dist/` - Build output (upload these files to Portal Web Editor)

## Development Notes

- No hot reload - must rebuild, upload bundle.ts and bundle.strings.json to Portal Web Editor, restart server and test
